...
async def main():
    ...
    light_matrix.show_image(light_matrix.IMAGE_PITCHFORK)
...
async def main():
    # Show a built-in happy face image on the display
    # All image names start with light_matrix.IMAGE_
    light_matrix.show_image(light_matrix.IMAGE_RABBIT)
...
# Wait 2 seconds (2000 milliseconds) so we can see the image
    # Always use "await" before runloop.sleep_ms()
    await runloop.sleep_ms(2000)
    light_matrix.write("Hello today is friday my name is man bun")
runloop.run(main())